...
class DirScan:
    ...
    def __init__(self, host_name='http://www.baidu.com', file_path='./dirscan/dictionary/'):
        self.scan_host_name = host_name
        self.queues = queue.Queue()
        self.file_list = self.get_all_file(file_path=file_path)
        self.total_keys = self.calculate_total_keys()  # 计算总任务数
        self.scanned_keys = 0  # 已扫描的任务数

    def calculate_total_keys(self):
        """计算总任务数（字典中的条目数）"""
        total = 0
    ...
    def scan_dir_worker(self):
        while True:
            ...
            self.scan_dir(key)
            self.scanned_keys += 1
            progress = int((self.scanned_keys / self.total_keys) * 100)  # 计算进度
            cache.set("scan_progress_dirscan", progress, timeout=3600)  # 更新缓存
            print(progress)
